...
import NewDynamicLayout from "./LockersJson/NewDynamicLayout.json";

import "./dynamicLayoutDesign.css";
...
const DynamicLayout = (props) => {
  ...
  const [lockerDetails, setLockerDetails] = useState({
    // seat: Lockers.AHCEB2F,
    seat: NewDynamicLayout[props.terminalID].lockers,
  ...
  const displayChooseLocks = () => {
    console.log(language.LockerReservationPage.chooseLocksAgain);
...
function DrawGrid(props) {
  ...
  const dynamicLockerLayout = NewDynamicLayout[terminalID].layout;
  ...
  function renderAsLayout() {
    ...
    for (let i = 0; i < dynamicLockerLayout.length; i++) {
      // console.log(i);
      ...
      for (let j = 0; j < dynamicLockerLayout[i].length; j++) {
        // console.log(lockerObject.lockValue[i][j]);

        console.log("inside second for loop");

        const lockerArr = dynamicLockerLayout[i][j].split("#");
      ...
      <button
        ...
        className={
          lockName === "QR"
            ...
            : props.userSelected === lockName
            ? "dynamiclayout-selected"
            : props.available.indexOf(lockName) > -1
            ? "dynamiclayout-available"
            : "dynamiclayout-reserved"
        }
        key={keyVal}
        onClick={
          props.checktrue(lockName) ? (e) => onClickSeat(lockName) : null
        }
      >
        {lockName}
      </button>
    ...
    <>
      <div>
        ...
        >
          ...
          <DialogActions>
            <Button
              onClick={closeImgSuggestionWindow}
              variant="outlined"
              color="primary"
            >
              ok
      ...
      <Col>
        ...
        >
          <div className="grid-container">
            ...
            <div className="dynamic-layout-border">{renderAsLayout()}</div>
          </div>
